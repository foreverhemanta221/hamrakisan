...
@section('body')
    <section class="content-header">
        <h1>
            {{$trainingDetail->training_name}}'s Syllabus
            <small>Syllabus Contents</small>
        </h1>
        <ol class="breadcrumb">
    ...
    <section class="content">
        <div class="box box-body">
            <div class="box-header">FAQs
                <a href="{{route('syllabus.create',$trainingDetail->id)}}" class="btn btn-success pull-right">Add Syllabus</a>
            </div>
            @if($trainingDetail->rel_syllabus->count()>0)
                @foreach($trainingDetail->rel_syllabus as $syllabus)
                    <div class="box">
                        <div class="box-header with-border">
                            <h3 class="box-title">{{$syllabus->training_syllabus_title}}</h3>
                        ...
                        <div class="box-body" style="">
                            {{$syllabus->training_syllabus_detail}}
                            <hr>
                            <strong>{{$syllabus->training_syllabus_duration}}</strong>
                        </div>
                    </div>
                @endforeach
            @else
