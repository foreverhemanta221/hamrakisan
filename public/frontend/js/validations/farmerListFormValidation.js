...
function checkInputs(e) {
    ...
    console.log(
        farmSpecificAddressValue !== "" && farmSpecificAddressValue.length < 6
    );
    e.preventDefault();
    if (
        farmSpecificAddressValue !== "" &&
        farmSpecificAddressValue.length < 6
    ) {
        e.preventDefault();
        errorMessage(
            farmSpecificAddress,
            "Specific Address must have six character long",
            "errSpecificAddress"
        );
    } else {
